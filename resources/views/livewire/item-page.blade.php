    ...
    <header class="pt-24 bg-[#162131]">
            @foreach ($categories as $category)
                @if (is_null($category->category_id))
                    <div>
                        <img class="object-cover h-auto w-full" src={{ asset('storage/' . $category->featured) }} alt="">
                    </div>
                @endif
            @endforeach
        </header>
            ...
            <div class="w-auto">
                ...
                <div class="flex items-center justify-between gap-6 py-8">
                    <div class="flex items-center gap-4">
                        <del class="text-[#CA0156] text-xs font-semibold ">{{ $product->valnormal }}</del>
                        <span class="text-blue-800 text-lg font-semibold ">{{ $product->valpromo }}</span>
                    </div>
